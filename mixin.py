...
class NODEMixin:
    """
    Mixin class providing Neural Ordinary Differential Equation functionality.
    
    This mixin enables the integration of Neural ODEs into VAE architectures,
    allowing for the modeling of continuous-time dynamics in latent space.
    Neural ODEs are particularly useful for modeling temporal processes and
    ensuring smooth latent trajectories.
    """
    ...
    def get_step_size(
        ...
        """
        Configure step size for ODE solver based on input parameters.
        
        Parameters
        ----------
        step_size : Optional[float]
            Desired step size. If None, uses adaptive stepping.
            If "auto", computes based on time range and number of points.
        t0 : float
            Starting time point
        t1 : float
            Ending time point
        ...
        -------
        Dict[str, Any]
            Configuration dictionary for the ODE solver
        """
        if step_size is None:
            return {}
        else:
            if step_size == "auto":
                computed_step_size = (t1 - t0) / (n_points - 1)
                return {"step_size": computed_step_size}
            return {"step_size": step_size}

    def solve_ode(
        self,
        ode_function: torch.nn.Module,
        ...
        """
        Solve ordinary differential equation using torchdiffeq.
        
        This method integrates the given ODE function from the initial state
        across the specified time points, returning the trajectory in latent space.
        
        Parameters
        ----------
        ode_function : torch.nn.Module
            Neural network defining the ODE dynamics f(t, z)
        initial_state : torch.Tensor
            Initial condition z₀ at t₀
        time_points : torch.Tensor
            Time points at which to evaluate the solution
        method : str, default="rk4"
            Numerical integration method (e.g., "rk4", "dopri5", "euler")
        step_size : Optional[float], default=None
            Step size for fixed-step methods
            
        Returns
        -------
        torch.Tensor
            Solution trajectory with shape (n_time_points, *initial_state.shape)
        """
        solver_options = self.get_step_size(
            step_size, time_points[0], time_points[-1], len(time_points)
        )

        # Move to CPU for numerical stability with certain solvers
        cpu_initial_state = initial_state.to("cpu")
        cpu_time_points = time_points.to("cpu")
        ...
        solution_trajectory = solution_trajectory.to(initial_state.device)
