...
def quiver_autoscale(embeddings: np.ndarray, velocities: np.ndarray) -> float:
    """
    Compute automatic scaling factor for quiver plots.
    
    This function determines the appropriate scale factor for visualizing
    vector fields using matplotlib's quiver function.
    
    Args:
        embeddings: 2D array of point coordinates (n_points, 2)
        velocities: 2D array of velocity vectors (n_points, 2)
    ...
    fig, ax = plt.subplots()
    scale_factor = np.abs(embeddings).max()

    # Create quiver plot with normalized coordinates
    quiver_plot = ax.quiver(
        embeddings[:, 0] / scale_factor,
        embeddings[:, 1] / scale_factor,
        velocities[:, 0],
        velocities[:, 1],
        angles="xy",
        scale=None,
    ...
    # Initialize and extract scale
    quiver_plot._init()
    optimal_scale = quiver_plot.scale / scale_factor
...
def l2_norm(x: Union[ndarray, csr_matrix], axis: int = -1) -> ndarray:
    """
    Compute L2 (Euclidean) norm with support for sparse matrices.
    
    Args:
        x: Input array (dense or sparse matrix)
        axis: Axis along which to compute the norm
        
    Returns:
        L2 norm values as NumPy array
    """
    if issparse(x):
