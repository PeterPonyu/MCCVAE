    ...
    def sample(self, sample_shape=torch.Size()):
        ...
        with torch.no_grad():
            ...
            for i in range(60):
                ...
                # Check for NaN or if gradient is negative (good)
                if torch.isnan(g_right).any():
                    # If we get NaN, we've gone too far - back off
                    xi[:, -1:] = xi[:, -1:] / 1.5
                    continue
                if (g_right < 0).all():
                    break
                # Expand more conservatively to avoid overflow
                xi[:, -1:] = (xi[:, -1:] * 1.5).clamp(max=1e5)

            # Final sanity: if still not bracketed, use fallback
            g_left = self.grad_log_prob(xi[:, :1])
            g_right = self.grad_log_prob(xi[:, -1:])
            
            # Replace NaN gradients with safe fallback values
            if torch.isnan(g_left).any() or torch.isnan(g_right).any():
                # Fall back to simple Gaussian sampling when bracketing fails
                value = mean + stddev * torch.randn(*sample_shape, *mean.shape, device=self.device).clamp(min=eps)
                return value
            
            if not (g_left > 0).all() or not (g_right < 0).all():
                # Fall back to simple Gaussian sampling
                value = mean + stddev * torch.randn(*sample_shape, *mean.shape, device=self.device).clamp(min=eps)
                return value
