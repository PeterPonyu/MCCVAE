...
class Distribution():
    def __init__(self, mean, sigma) -> None:
        self.mean = mean
        # Clamp sigma to prevent numerical overflow in hyperbolic radius sampling
        # Keep consistent with encoder bounds (min 0.01 for stability, max 10 to prevent overflow)
        self.sigma = sigma.clamp(min=0.01, max=10.0)
